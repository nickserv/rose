  ...
  describe('.search()', () => {
    ...
    context('with an empty query', () => {
      it('finds all features', () => {
        return expect(feature.search(seedData, '')).to.have.length(7);
      });
    });

    context('with a feature query', () => {
      it('finds all matching features', () => {
        return expect(feature.search(seedData, 'add files')).to.eql(gitAddFeature);
      });
    });

    context('with a technology query', () => {
      it('finds all matching features', () => {
        return expect(feature.search(seedData, 'git')).to.have.length(6);
      });
    });

    context('with a command query', () => {
      it('finds all matching features', () => {
        return expect(feature.search(seedData, 'git ADD')).to.eql(gitAddFeature);
      });
    });

    context('with a command query for a command that does not exist', () => {
      it('finds no features', () => {
        return expect(feature.search(seedData, 'git yolo')).to.have.length(0);
      });
    });
  });
});
